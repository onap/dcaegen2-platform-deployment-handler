...
const DELAY_DEPLOYMENT_UPDATE = 5000;
...
// Go through the Cloudify API call sequence to upload blueprint and call perform deployment update
// (but don't wait for the update to finish)
const launchBlueprintForUpdate = function(req, id, blueprint, bpTypeId, inputs) {
	logger.info(req.dcaeReqId, "deploymentId: " + id + " starting blueprint upload");

	return cfy.checkBlueprintExits(req, bpTypeId)

		.then (function (result) {
			logger.info(req.dcaeReqId, " checkBlueprintExits result " + result);
			// Upload blueprint
			if ( result === false ) {
				return cfy.uploadBlueprint(req, bpTypeId, blueprint)
			}
		})
		...
		.then (function() {
			...
			// Update deployment
			return cfy.updateDeployment(req, id, bpTypeId, inputs);
		})

		.catch(function(error) {
			logger.info(req.dcaeReqId, "Error: " + JSON.stringify(error) + " for launch blueprint for deploymentId " + id + " with blueprint id: " + bpTypeId);
			throw normalizeError(error);
		});
};
exports.launchBlueprintForUpdate = launchBlueprintForUpdate;
...
const finishDeploymentUpdate = function(req, deploymentId, executionId) {
	logger.info(req.dcaeReqId, "finishDeploymentUpdate: " + deploymentId + " -- executionId: " + executionId);
	return cfy.waitForWorkflowExecution(req, executionId)
		.then (function(result){
			logger.info(req.dcaeReqId, "deploymentId: " + deploymentId + " deployment updated executed");
		})
		.catch(function(err) {
			logger.info(req.dcaeReqId, "Error finishing install workflow: " + err + " -- " + JSON.stringify(err));
			throw normalizeError(err);
...
exports.finishDeploymentUpdate = finishDeploymentUpdate;
