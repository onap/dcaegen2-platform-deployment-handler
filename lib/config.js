...
const getConfigFromEnv = function() {
	// Without consul, all configuration will be from environment vars but we can later set some initial config here
	return {};
};

/* Fetch configuration from consi;*/
const getConfigFromConsul = function() {
...
/* Get a service from environment */
const getServiceFromEnv = function (serviceName) {
...
/* Get a service host:port from consul */
const getServiceFromConsul = function (serviceName) {
...
const checkEnvVars = function() {
	...
	if ( process.env.CLOUDIFY ) {
		...
		log.info('',"CLOUDIFY = " + process.env.CLOUDIFY);
	}
	else {
		console.log("\t -> No CLOUDIFY env var defined.");
		log.info('',"No CLOUDIFY env var defined.");
		missingEnvVars = 'CLOUDIFY';
	}
	...
	if ( process.env.INVENTORY_SERVICE_SERVICE_PORT  ) {
		...
		log.info('',"INVENTORY_SERVICE_SERVICE_PORT = " + process.env.INVENTORY_SERVICE_SERVICE_PORT);
	}
	else {
		console.log("\t -> No INVENTORY_SERVICE_SERVICE_PORT env var defined.");
		log.info('', "No INVENTORY_SERVICE_SERVICE_PORT env var defined");
		missingEnvVars += ', INVENTORY_SERVICE_SERVICE_PORT';
	}

	if ( process.env.INVENTORY_PROTOCOL ) {
		console.log("\t -> INVENTORY_PROTOCOL = " + process.env.INVENTORY_PROTOCOL );
		log.info('', "INVENTORY_PROTOCOL = " + process.env.INVENTORY_PROTOCOL);
	}
	else {
		console.log("\t -> No process.env.INVENTORY_PROTOCOL env var defined.");
		log.info('', "No process.env.INVENTORY_PROTOCOL env var defined.");
		missingEnvVars += ', INVENTORY_PROTOCOL';
	}

	if ( process.env.INVENTORY_SERVICE_NAME ) {
		console.log("\t -> INVENTORY_SERVICE_NAME = " + process.env.INVENTORY_SERVICE_NAME );
		log.info('', "INVENTORY_SERVICE_NAME = " + process.env.INVENTORY_SERVICE_NAME);
	}
	else {
		console.log("\t -> No process.env.INVENTORY_SERVICE_NAME env var defined.");
		log.info('', "No process.env.INVENTORY_SERVICE_NAME env var defined.");
		missingEnvVars += ', INVENTORY_SERVICE_NAME';
	}

	if ( process.env.NAMESPACE ) {
		console.log("\t -> NAMESPACE = " + process.env.NAMESPACE);
		log.info('', "NAMESPACE = " + process.env.NAMESPACE);
	}
	else {
		console.log("\t -> No process.env.NAMESPACE env var defined.");
		log.info('', "No process.env.NAMESPACE env var defined.");
		missingEnvVars += ', NAMESPACE';
	}
	...
	if ( process.env.INVENTORY_SERVICE_NAME_SUFFIX ) {
		...
		log.info('', "INVENTORY_SERVICE_NAME_SUFFIX = " + process.env.INVENTORY_SERVICE_NAME_SUFFIX);
	}
	else {
		console.log("\t -> No process.env.INVENTORY_SERVICE_NAME_SUFFIX env var defined.");
		log.info('', "No process.env.INVENTORY_SERVICE_NAME_SUFFIX env var defined.");
		missingEnvVars += ', INVENTORY_SERVICE_NAME_SUFFIX';
	}
...
exports.configure = function() {
	...
	console.log((new Date()) + " Environment var USE_CONSUL = " + process.env.USE_CONSUL);

	if ( process.env.USE_CONSUL ) {
		useConsul = (process.env.USE_CONSUL.toLowerCase() === 'true');
	}
	config.useConsul = useConsul;

	console.log( (new Date()) + ": config.configure(): useConsul " + useConsul );

	const getConfig = (useConsul === false) ? getConfigFromEnv : getConfigFromConsul;
	const getService = (useConsul === false) ? getServiceFromEnv : getServiceFromConsul;
	...
	.then(function(package_json) {
        ...
        if ( useConsul === false ) {
			/* Check for missing required configuration environment variables when we're not using consul */
			const missingEnvVars = checkEnvVars();
			if (missingEnvVars.length > 0) {
				throw new Error ("Required configuration environments var(s) missing: " + missingEnvVars);
			}
		}
		else {
			console.log( (new Date()) + " Environment var CONSUL_HOST = " + process.env.CONSUL_HOST );
		}
